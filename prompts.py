...
AGENT_SYSTEM_PROMPT = (
    ...
    "  * Telegram: delivery_channel='telegram', delivery_target='<chat_id>'\n"
    "    where <chat_id> is a numeric Telegram chat ID (e.g. '123456789').\n"
    "    If the user doesn't know their chat ID, tell them to message the bot\n"
    "    and check the Thoth log or use @userinfobot on Telegram.\n"
    "  * Email: delivery_channel='email', delivery_target='user@example.com'.\n"
    "  * If the user asks for delivery but doesn't specify a target, ASK them\n"
    "    for the chat ID or email address — do NOT guess.\n"
